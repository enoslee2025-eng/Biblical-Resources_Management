...
function parseEntries(text) {
  ...
  /* 经文引用正则：支持 创1:1、创世记1:1、Gen 1:1 等格式 */
  const refPattern = /^(.{1,15}?\d+[:：]\d+(?:[-–]\d+)?)\s*[：:—\-]?\s*(.*)/
  ...
  for (const line of lines) {
    ...
    if (match) {
      ...
      /* 尝试匹配 "标题：内容" 格式 */
      const simpleMatch = line.match(/^(.{1,30})[：:\\-—]\s*(.+)/)
      if (simpleMatch) {
        entries.push({
          reference: simpleMatch[1].trim(),
...
function confirmImport() {
  sessionStorage.setItem('commentaryImportData', JSON.stringify(parsedEntries.value))
    ...
    <div v-if="showPreview" class="preview-section">
      ...
      <div class="preview-list">
        <div v-for="(entry, idx) in parsedEntries.slice(0, 50)" :key="idx" class="preview-entry">
          <div class="entry-ref" v-if="entry.reference">{{ entry.reference }}</div>
      ...
      <div class="preview-actions">
        ...
        <el-button type="primary" @click="confirmImport">{{ t('import_commentary_confirm') }}</el-button>
      </div>
    </div>
...
.import-card:hover { border-color: #52c41a; box-shadow: 0 8px 24px rgba(82, 196, 26, 0.12); transform: translateY(-4px); }
...
.entry-ref { font-size: 14px; font-weight: 600; color: #52c41a; margin-bottom: 4px; }
